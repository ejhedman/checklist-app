-- Members Table Redo Migration (Safe Version)
...
-- STEP 3: Drop existing foreign key constraints that reference members.member_id
...
-- STEP 4: Temporarily drop the role constraint
ALTER TABLE members DROP CONSTRAINT IF EXISTS members_role_check;

-- STEP 5: Rename member_id column to user_id
ALTER TABLE members RENAME COLUMN member_id TO user_id;

-- STEP 6: Update existing roles from 'user' to 'member'
UPDATE members SET role = 'member' WHERE role = 'user';

-- STEP 7: Add the new role constraint
ALTER TABLE members ADD CONSTRAINT members_role_check 
  CHECK (role IN ('member', 'release_manager', 'admin'));

-- STEP 8: Verify roles were updated correctly
SELECT 
...
-- STEP 9: Recreate foreign key constraints with the new user_id column
...
-- STEP 10: Update RLS policies to use the new column name
...
-- STEP 11: Create a function to get auth user by email for the autocomplete
...
-- STEP 12: Create a function to create a member from an auth user
...
-- STEP 15: Test the auth user lookup function
SELECT 
...
-- STEP 16: Show available auth users for member creation
SELECT 

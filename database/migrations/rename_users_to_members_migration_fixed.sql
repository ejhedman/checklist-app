-- Migration: Rename users table to members and add member_id field (FIXED VERSION)
-- This script will rename the users table to members and update all references
-- Run this in your Supabase SQL editor

-- STEP 0: Ensure the set_updated_at function exists
...
-- STEP 6: Create triggers for the new tables (only after ensuring tables exist)
...
DROP TRIGGER IF EXISTS set_member_release_state_updated_at ON member_release_state;
CREATE TRIGGER set_member_release_state_updated_at 
  BEFORE UPDATE ON member_release_state
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
DROP POLICY IF EXISTS "Members can view team memberships" ON team_members;
CREATE POLICY "Members can view team memberships" ON team_members
FOR SELECT USING (auth.role() = 'authenticated');

-- Allow authenticated users to manage team memberships
DROP POLICY IF EXISTS "Members can manage team memberships" ON team_members;
...
-- STEP 13: Verify the migration before dropping old tables
SELECT 
...
-- STEP 14: Show sample data to verify migration
SELECT 
...
-- STEP 15: Drop old tables and columns (ONLY AFTER VERIFYING DATA MIGRATION)
-- Uncomment the following lines ONLY after you've verified the migration was successful
